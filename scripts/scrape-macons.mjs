/**
 * Script de scraping des maçons en France (Sans clé d'API Google Places).
 * 
 * Ce script navigue directement sur le site public Google Maps à l'aide de Puppeteer,
 * extrait les maçons et entreprises de maçonnerie/rénovation, clique sur chaque établissement 
 ...
 *   - Mots-clés : "macon", "entreprise maconnerie", "renovation"
 *   - Fichier de sortie : outputs/macons_france.csv
 */
...
const OUTPUT_FILE = path.join(OUTPUT_DIR, "macons_france.csv");
const PROCESSED_IDS_FILE = path.join(OUTPUT_DIR, "macons_processed_ids.txt");
...
async function main() {
  console.log("🚀 Lancement du scraping des maçons (SANS CLÉ API)");
  ...
  try {
    ...
    for (let c = 0; c < citiesToProcess.length; c++) {
      ...
      const searchUrl = `https://www.google.com/maps/search/macon+entreprise+maconnerie+renovation+${encodeURIComponent(city)}/`;
      try {
      ...
      console.log(`  🎯 ${placesToScrape.length} nouveaux maçons à analyser.`);
