/**
 * Script de scraping des plombiers en France - EXTENSION (Villes 101 à 200).
 * 
 * Ce script navigue directement sur le site public Google Maps à l'aide de Puppeteer,
 * extrait les plombiers-chauffagistes, clique sur chaque établissement pour en
 ...
 *   - Note Google >= 4.0
 *   - Nombre d'avis >= 10
 *   - Cible les 100 villes françaises suivantes par population (101 à 200)
 *   - Mots-clés : "plombier" et "chauffagiste"
 *   - Fichier de sortie : outputs/plombiers_france.csv (ajout à la fin)
 */
...
const OUTPUT_FILE = path.join(OUTPUT_DIR, "plombiers_france.csv");
const PROCESSED_IDS_FILE = path.join(OUTPUT_DIR, "plombiers_processed_ids.txt");
const processedPlaceIds = new Set();

// Les 100 villes suivantes par population (rangs 101 à 200)
const CITIES = [
  "Nouméa", "Le Tampon", "Fort-de-France", "Drancy", "Mamoudzou", "Évry-Courcouronnes", "Le Blanc-Mesnil", 
  "Pantin", "Fréjus", "Saint-André", "Clamart", "Narbonne", "Bobigny", "Meaux", "Maisons-Alfort", "Saint-Louis", 
  "La Roche-sur-Yon", "Chelles", "Corbeil-Essonnes", "Saint-Laurent-du-Maroni", "Cholet", "Bayonne", 
  "Fontenay-sous-Bois", "Saint-Ouen-sur-Seine", "Cagnes-sur-Mer", "Vaulx-en-Velin", "Épinay-sur-Seine", 
  "Sartrouville", "Sevran", "Arles", "Massy", "Albi", "Les Abymes", "Gennevilliers", "Saint-Herblain", 
  "Grasse", "Bondy", "Les Sables-d'Olonne", "Laval", "Évreux", "Saint-Priest", "Suresnes", "Martigues", 
  "Vincennes", "Aubagne", "Livry-Gargan", "Blois", "Rosny-sous-Bois", "La Courneuve", "Brive-la-Gaillarde", 
  "Bastia", "Talence", "Meudon", "Montrouge", "Alès", "Carcassonne", "Melun", "Choisy-le-Roi", 
  "Saint-Germain-en-Laye", "Belfort", "Charleville-Mézières", "Alfortville", "Noisy-le-Sec", "Sète", 
  "Chalon-sur-Saône", "Bagneux", "Tarbes", "Saint-Brieuc", "Istres", "Salon-de-Provence", "Puteaux", 
  "Caluire-et-Cuire", "Rezé", "Mantes-la-Jolie", "Valenciennes", "Anglet", "Bagnolet", "Bron", 
  "Châlons-en-Champagne", "Châteauroux", "Arras", "Thionville", "Villenave-d'Ornon", "Castres", 
  "Bourg-en-Bresse", "Gagny", "Le Cannet", "Angoulême", "Garges-lès-Gonesse", "Villepinte", "Stains", 
  "Gap", "Poissy", "Colomiers", "Wattrelos", "Compiègne", "Montélimar", "Boulogne-sur-Mer", "Douai", 
  "Marcq-en-Barœul"
];
...
async function main() {
  console.log("🚀 Lancement du scraping des plombiers (EXTENSION 101-200, SANS CLÉ API)");
  ...
  try {
    ...
    for (let c = 0; c < citiesToProcess.length; c++) {
      ...
      const searchUrl = `https://www.google.com/maps/search/plombier+chauffagiste+${encodeURIComponent(city)}/`;
      try {
      ...
      console.log(`  🎯 ${placesToScrape.length} nouveaux plombiers à analyser.`);
