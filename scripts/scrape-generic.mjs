/**
 * Script générique de scraping de professionnels en France (SANS CLÉ API).
 * 
 * Ce script navigue sur Google Maps via Puppeteer, recherche des professionnels
 * pour une catégorie donnée dans les 100 plus grandes villes de France,
 * et extrait leurs coordonnées (adresse, téléphone, site web) ainsi que
 * leurs emails (en crawlant leurs sites web).
 * 
 * Usage :
 *   node scripts/scrape-generic.mjs --slug comptabilite --query "expert comptable cabinet comptable"
 *   node scripts/scrape-generic.mjs --slug jardinage --query "paysagiste jardinier elagage" --limit-cities 10
 *   node scripts/scrape-generic.mjs --slug assurance --query "courtier assurance agence assurance" --city "Lille"
 */
...
if (!slug || !query) {
  console.error("❌ Arguments requis : --slug <slug_categorie> --query <mots_cles_recherche>");
  console.error("Exemple : node scripts/scrape-generic.mjs --slug comptabilite --query \"expert comptable cabinet comptable\"");
  process.exit(1);
}

const OUTPUT_FILE = path.join(OUTPUT_DIR, `${slug}_france.csv`);
const PROCESSED_IDS_FILE = path.join(OUTPUT_DIR, `${slug}_processed_ids.txt`);
...
async function main() {
  console.log(`🚀 Lancement du scraping générique : '${slug}'`);
  ...
  try {
    ...
    for (let c = 0; c < citiesToProcess.length; c++) {
      ...
      console.log(`  🎯 ${placesToScrape.length} nouveaux professionnels à analyser.`);
      ...
      for (let i = 0; i < placesToScrape.length; i++) {
        ...
        await sleep(1000); // Petite pause entre chaque clic
      }
  ...
  console.log("============================================================");
  console.log("🏁 Scraping générique terminé !");
