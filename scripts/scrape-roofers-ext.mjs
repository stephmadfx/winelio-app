/**
 * Script de scraping des couvreurs en France - EXTENSION (Villes 101 à 200).
 * 
 * Ce script navigue directement sur le site public Google Maps à l'aide de Puppeteer,
 * extrait les établissements de couverture, clique sur chaque établissement pour en
 ...
 *   - Mots-clés : "couvreur"
 *   - Fichier de sortie : outputs/couvreurs_france.csv (ajout à la fin)
 */
...
const OUTPUT_FILE = path.join(OUTPUT_DIR, "couvreurs_france.csv");
const PROCESSED_IDS_FILE = path.join(OUTPUT_DIR, "couvreurs_processed_ids.txt");
...
async function main() {
  console.log("🚀 Lancement du scraping des couvreurs (EXTENSION 101-200, SANS CLÉ API)");
  ...
  try {
    ...
    for (let c = 0; c < citiesToProcess.length; c++) {
      ...
      const searchUrl = `https://www.google.com/maps/search/couvreur+${encodeURIComponent(city)}/`;
      try {
      ...
      console.log(`  🎯 ${placesToScrape.length} nouveaux couvreurs à analyser.`);
