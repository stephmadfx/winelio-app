/**
 * Script de scraping des installateurs de climatisation en France (Sans clé d'API Google Places).
 * 
 * Ce script navigue directement sur le site public Google Maps à l'aide de Puppeteer,
 * extrait les installateurs de climatisation, clique sur chaque établissement pour en
 ...
 *   - Mots-clés : "climatisation", "installateur clim"
 *   - Fichier de sortie : outputs/climatisation_france.csv
 */
...
const OUTPUT_FILE = path.join(OUTPUT_DIR, "climatisation_france.csv");
const PROCESSED_IDS_FILE = path.join(OUTPUT_DIR, "climatisation_processed_ids.txt");
...
async function main() {
  console.log("🚀 Lancement du scraping de la climatisation (SANS CLÉ API)");
  ...
  try {
    ...
    for (let c = 0; c < citiesToProcess.length; c++) {
      ...
      const searchUrl = `https://www.google.com/maps/search/climatisation+installateur+clim+${encodeURIComponent(city)}/`;
      try {
      ...
      console.log(`  🎯 ${placesToScrape.length} nouveaux installateurs de climatisation à analyser.`);
