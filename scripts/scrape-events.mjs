/**
 * Script de scraping des professionnels de l'événementiel (Sans clé d'API Google Places).
 * 
 * Ce script navigue directement sur le site public Google Maps à l'aide de Puppeteer,
 * extrait les wedding planners, DJs et agences événementielles, clique sur chaque établissement 
 * pour en lire les détails (adresse, téléphone, site web) et crawl les sites web pour
 ...
 *   - Mots-clés : "wedding planner", "DJ mariage", "agence evenementielle"
 *   - Fichier de sortie : outputs/evenementiel_france.csv
 */
...
const OUTPUT_FILE = path.join(OUTPUT_DIR, "evenementiel_france.csv");
const PROCESSED_IDS_FILE = path.join(OUTPUT_DIR, "evenementiel_processed_ids.txt");
...
async function main() {
  console.log("🚀 Lancement du scraping de l'événementiel (SANS CLÉ API)");
  ...
  try {
    ...
    for (let c = 0; c < citiesToProcess.length; c++) {
      ...
      const searchUrl = `https://www.google.com/maps/search/wedding+planner+DJ+mariage+agence+evenementielle+${encodeURIComponent(city)}/`;
      try {
      ...
      console.log(`  🎯 ${placesToScrape.length} nouveaux établissements à analyser.`);
