/**
 * Script de scraping des plombiers en France (Sans clé d'API Google Places).
 * 
 ...
 *   - Fichier de sortie : outputs/plombiers_france.csv
 * 
 * Usage:
 *   node scripts/scrape-plumbers.mjs
 *   node scripts/scrape-plumbers.mjs --limit-cities 3
 *   node scripts/scrape-plumbers.mjs --city Lille
 *   node scripts/scrape-plumbers.mjs --skip-puppeteer-fallback
 */
...
async function main() {
  console.log("🚀 Lancement du scraping des plombiers (SANS CLÉ API)");
  ...
  try {
    ...
    for (let c = 0; c < citiesToProcess.length; c++) {
      ...
      // On combine les requêtes plombier & chauffagiste pour optimiser le temps
